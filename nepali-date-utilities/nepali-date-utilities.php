...
Description: A Nepali Date Utitlities plugin for wordpress. The plugin contains post dates and today's date shortcode to help sites display Nepali dates.
Version: 1.0.15
Author: Ashok Basnet
Author URI: http://ashokbasnet.com.np
*/
if (!defined('NPD_BASENAME'))
    define('NPD_BASENAME', plugin_basename(__FILE__));
if (!defined('NPD_PLUGIN_URL'))
    define('NPD_PLUGIN_URL', plugin_dir_url(__FILE__));
if (!defined('NPD_PLUGIN_DIR'))
    define('NPD_PLUGIN_DIR', plugin_dir_path(__FILE__));
function ndu_init()
{
...
function ndu_nepali_date_utilities_page()
{
    if (array_key_exists('submit_ndu_settings', $_POST)) {
        ...
        <div id="setting-error-settings_updated" class="update_settings-error notice is-dismissible">
            <p>
                <strong>Yay !!! Settings has been saved.</strong>
            </p>
        </div>
        <?php
    }
    ...
    $convertPostDate = get_option('ndu_convert_post_date', '1');
    ?>
    <h1>Nepali Date Utilties Plugin Settings</h1>
    <hr>

    <div class="wrap">
        <form method="POST">
            <label name="ndu_convert_post_date">Convert Post Date to Nepali:</label>
            <input type="checkbox" name="ndu_convert_post_date" <?php echo ($convertPostDate == "1") ? 'checked' : ''; ?> style="margin-top: 0px;"/>
            <br/><br/>
            <label for="ndu_date_format">Post Date Format( y m d l H/h i s a)
            </label>
    ...
    <ol>
        <li>
            <code><?php echo htmlspecialchars('<?php echo ndu_today_date();?>'); ?></code>
            to print today's date.
        </li>
        <li>
            <code><?php echo htmlspecialchars("<?php echo do_shortcode('[ndu_today_date]');?>"); ?></code>
            to print today's
                                                            date using shortcode.
        </li>
    </ol>
    <hr>
    ...
    <p>
        "Nepali Date Utilities" plugin is developed in order to provide necessary dates conversion from English to Nepali
                                        dates. The plugin contains post dates and today's date shortcode to help sites display nepali dates.
    </p>
    <a href="https://wordpress.org/plugins/nepali-date-utilities" target="_blank">Go to plugin page</a>
    /
    <a href="mailto:mail@ashokbasnet.com.np">
        Provide Feedback</a>
